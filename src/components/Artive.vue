	...
	<div id="artive">
		...
		<div class="content">
		    ...
		    <div class="cont">
		        ...
		        <div class="time"><p>{{articleData.time | normalDate}} <span><img src="../assets/img/zan.png" alt=""/></span></p></div>
...
<script>
	export default{
		...
		methods:{
			fetchData(ind){
				this.axios.get('../src/data/article.data').then((response) =>{
					this.articleData = response.data[ind - 1]
				}).catch((error)=>{
					console.log(error)
				})
			}
